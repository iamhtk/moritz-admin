...
Meters carry `role="meter"` with value attributes. Charts carry `role="img"` with descriptions. The assistant answer is an `aria-live` polite region. Capacity always shows a ratio (and plain state text when High or Over), never colour alone; the percentage is in the tooltip.
